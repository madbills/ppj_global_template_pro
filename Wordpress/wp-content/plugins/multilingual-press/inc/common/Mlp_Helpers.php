...
class Mlp_Helpers {
	...
	public static function show_linked_elements( $args ) {
		...
		$bloglang = explode('-', get_bloginfo('language'));
		$langCode = gettype($bloglang) === 'array' ? $bloglang[0] : $bloglang;

		$langName = $items[get_current_blog_id()]['name'];

		$output = '<div class="btn-group btn-lang pull-right">
			<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
				<i class="visible-xs-inline-block">'.$langCode.'</i><i class="hidden-xs">'.$langName.'</i>
