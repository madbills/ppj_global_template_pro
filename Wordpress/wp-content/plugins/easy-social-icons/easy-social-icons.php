...
Description: You can upload your own social icon, set your social URL, choose weather you want to display vertical or horizontal. You can use the shortcode <strong>[cn-social-icon]</strong> in page/post, template tag for php file <strong>&lt;?php if ( function_exists('cn_social_icon') ) echo cn_social_icon(); ?&gt;</strong> also you can use the widget <strong>"Social Icons"</strong> for sidebar. Red Balloon custom edited version of the original plugin.
Version: rb1.0.1(2.0.2)
Author: cybernetikz
...
# Red Balloon Required Updates:

* Change SQL query to only output icons with a url and (image url or fa class)
* Add example to add new icon page.

*/

/*

Red Balloon Change Log:

# Version 1.0.1

* Removed display options from admin
* Removed extra CSS and JS files from loading.
* Moved styles from style attribute to style element when outputting on front end.

# Version 1.0.0

* Added support for font awesome
* Updated the SQL table to include the font awesome information.
* For all updates search for "@RB: 1.0.0".
...
function cnss_admin_style() {
	...
	/*
	@RB: 1.0.1
	Updated files that are brought in.
	@RB: 1.0.0
	Added font awesome to admin
	*/
	if (wp_style_is('font-awesome', 'registered')){
		wp_enqueue_style('font-awesome');
	} else {
		wp_register_style('font-awesome', 'http://maxcdn.bootstrapcdn.com/font-awesome/4.3.0/css/font-awesome.min.css', false, '4.3.0' ); // Font awesome
		wp_enqueue_style('font-awesome');
	}
}
...
function cnss_add_menu_pages() {
	add_menu_page('Social Icons', 'Social Icons', 'manage_options', 'cnss_social_icon_page', 'cnss_social_icon_page_fn',plugins_url('/images/scc-sc.png', __FILE__) );
...
function cnss_social_icon_option_fn() {
	...
	<form method="post" action="options.php" enctype="multipart/form-data">
		...
		<table class="form-table">
			<tr valign="top">
			<th scope="row">Icon Height / Font size</th>
    ...
    <fieldset class="shadow">
        ...
        then drag drop <code>Social Icon</code> widget to <strong>Widget Area</strong></p>
    </fieldset>
    ...
    <fieldset class="shadow">
        ...
        <form method="post" action="admin.php?page=cnss_social_icon_option#shortcode" enctype="application/x-www-form-urlencoded">
            ...
            <table width="100%" border="0">
              <tr>
			    <td width="170"><label><?php _e( 'Icon Height/Font Size <em>(px)</em>:' ); ?></label>
...
function cnss_db_install () {
	...
	/*
	@RB: 1.0.0
	Added new columns, fa_class and fa_colour
	*/
			...
			if ($err == '')
			{
				...
				$results = $wpdb->insert(
					...
					array(
						...
						'fa_class' => sanitize_text_field($_POST['fa_class']),
						'fa_colour' => sanitize_text_field($_POST['fa_colour']),
	...
	<div class="wrap">
		...
		<div id="order-post-type" style="padding:15px 20px 20px; background:#fff; border:1px solid #ebebeb;">
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<thead>
					<th width="64"><span>Icon</span></th>
					<th width="200"><span>Title</span></th>
					<th><span>URL</span></th>
				</thead>
			</table>
			<?/*
			RB: 1.0.1
			Added styles for the sort page
			*/?>
			<style>
				.placeholder{
					width: 100%;
			...
			foreach($video_info as $vdoinfo) {
				if (strlen($vdoinfo->image_url) == 0 ) {
					/*
					@RB: 1.0.1
					Added new fallback image
					*/
					$image_url = 'https://placeholdit.imgix.net/~text?txtsize=16&txt=No+Image&w=64&h=64';
				} else {
			...
			?>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  <tr style="background:#f7f7f7">
						<td width="64" style="text-align:center; padding-top:4px;">&nbsp;<img src="<?php echo $image_url;?>" border="0" width="<?php echo $cnss_width ?>" height="<?php echo $cnss_height ?>" alt="<?php echo $vdoinfo->title;?>" /></td>
						<td width="200" style="text-align:center;"><span><?php echo $vdoinfo->title;?></span></td>
		...
		if ( $_GET['mode'] == 'edit' and $_GET['id'] != '' ) {
			...
			*/
			if (!empty($video_info))
			{
				...
				$fa_class = $video_info[0]->fa_class;
				$fa_colour = $video_info[0]->fa_colour;
    ...
    <table class="form-table">
        ...
        <tr valign="top">
			...
			<td>
				...
				<img style="vertical-align:top" id="logoimg" src="<?php echo $image_url==''?'https://placeholdit.imgix.net/~text?txtsize=16&txt=No+Image&w=64&h=64':$image_url; ?>" border="0" width="<?php echo $cnss_width; ?>"  height="<?php echo $cnss_height; ?>" alt="<?php echo $title; ?>" /><br />
			</td>
        </tr>

		<tr valign="top">
			<th scope="row">Font Awesome Class</th>
			<td>
				<input style="vertical-align:top" type="text" name="fa_class" id="fa_class" class="regular-text" value="<?php echo $fa_class ?>" />
				<div>
					<a href="http://fontawesome.io/icons/#search" target="_blank">Font Awesome Gallery</a>
				</div>
			</td>
        </tr>
		<tr valign="top">
			<th scope="row">Font Awesome Colour</th>
			<td>
				<input style="vertical-align:top" type="text" name="fa_colour" id="fa_colour" class="regular-text" value="<?php echo $fa_colour ?>" /> (optional)
			</td>
        </tr>
	...
	<div class="content_wrapper">
		...
		<table class="widefat page fixed" cellspacing="0">
			...
			<tr valign="top">
				...
				<th class="manage-column column-title" scope="col" width="100">Font Awesome</th> <?php //@RB: 1.0.0 Added fa_class and fa_colour ?>
			...
			foreach($video_info as $vdoinfo) {
				...
				/*
				@RB: 1.0.0
				Added fallback image for icons with font awesome instead of an image
				*/
				if (strlen($vdoinfo->image_url) == 0) {
					/*
					@RB: 1.0.1
			...
			<tr valign="top">
				...
				*/
				?>
				<td>
					<i class="social-plugin fa <?=$vdoinfo->fa_class?>" style="color:<?=$vdoinfo->fa_colour?>;height:<?= $cnss_height ?>; width:<?= $cnss_width ?>px; font-size:<?= $cnss_height?>px;"></i>
				</td>
...
function cn_social_icon_table() {
	...
	$table_name = $wpdb->prefix . "cn_social_icon";
	$image_file_path = $baseURL;
	$sql = "SELECT * FROM ".$table_name." WHERE url<>'' ORDER BY sortorder";
	$video_info = $wpdb->get_results($sql);
	...
	/*
	@RB: 1.0.0:
	If there isn't an image set use font awesome
	*/
	if (strlen($icon->image_url) == 0 ) {
		$fa_array = array(
			'class' => $icon->fa_class,
			'colour' => $icon->fa_colour
		);
		$image_url = '';
	} else {
	...
	?>
		<td style="width:<?php echo $td_width ?>px">
			<a <?php echo ($icon->target==1)?'target="_blank"':'' ?> title="<?php echo $icon->title ?>" href="<?php echo $icon->url ?>">
				<?php if ($image_url === ''): ?>
					<i class="social-plugin fa <?= $fa_array['class']?>" style="color:<?=$fa_array['colour']?>;font-size:<?= $cnss_height ?>px;"></i>
				<?php else:?>
					<img src="<?php echo $image_url?>" border="0" width="<?php echo $cnss_width ?>" height="<?php echo $cnss_height ?>" alt="<?php echo $icon->title ?>" />
				<?php endif; ?>
			</a>
		</td>
	<?php
...
function cn_social_icon($attr = array(), $call_from_widget = NULL) {
	...
	$table_name = $wpdb->prefix . "cn_social_icon";
	$image_file_path = $baseURL;
	$sql = "SELECT * FROM ".$table_name." WHERE url<>'' $where_sql ORDER BY sortorder";
	$video_info = $wpdb->get_results($sql);
	$icon_count = count($video_info);
	...
	/*
	@RB: 1.0.1
	Added ID to all outputs and moved the inline styles to a style tag as much as possible;
	*/
	static $iconOutputNo = 0;
	if (strlen($attr_id)>0) {
		$attr_id = $attr_id;
	} else{
		$attr_id = 'icons-auto-id-'.$iconOutputNo;
	}
	$iconOutputNo++;

	ob_start();
	echo '<ul id="'.$attr_id.'" class="cnss-social-icon '.$attr_class.'">';
	$i=0;
	...
	*/
	...
	?>
		<li class="<?php echo cnss_format_title($icon->title); ?>">
			<a <?php echo ($icon->target==1)?'target="_blank"':'' ?> title="<?php echo $icon->title ?>" href="<?php echo $icon->url ?>">
				<?php if ($image_url === ''): ?>
					<i class="social-plugin fa <?= $fa_array['class']?>" style="color:<?=$fa_array['colour']?>;"></i>
				<?php else:?>
					<?/*
					@RB: 1.0.1
					Removed inline styles as much as possible
					*/?>
					<img src="<?php echo $image_url; ?>" border="0" width="auto" alt="<?php echo $icon->title; ?>" style=" <?php echo $style_img; ?>" />
				<?php endif; ?>
			</a>
		</li>
	<?php
	...
	/*
	@RB: 1.0.1
	Add styles inline after the list is closed.
	*/
	$styles  = '<style>';
	$styles .= '/*Styles for icons that are image elements:*/';
	$styles .= '#'.$attr_id.'.cnss-social-icon li img{ width:'.$cnss_width.'px; height:'.$cnss_height.';}';
	$styles .= '/*Styles for icons that are font-awesome i elements:*/';
	$styles .= '#'.$attr_id.'.cnss-social-icon li i{ font-size:'.$cnss_height.'px;}';
	$styles .= '</style>';
...
function _cn_social_icon_sc( $selected_icons_array = array() ) {
		...
		foreach($video_info as $icon)
		{
			...
			*/
			...
			?><li style="display:inline-block; padding:2px 8px;">
	            <div align="center">
					<label for="icon<?php echo $icon->id; ?>">
						<?php if ( $image_url === '' ): ?>
							<i class="social-plugin fa <?= $fa_array['class']?>" style="color:<?=$fa_array['colour']?>;background:<?=$fa_array['background']?>; font-size:<?= $cnss_height?>px;"></i>
						<?php else: ?>
							<img src="<?php echo $image_url?>" border="0" width="<?php echo $cnss_width ?>" height="<?php echo $cnss_height ?>" alt="<?php echo $icon->title ?>" title="<?php echo $icon->title ?>" />
						<?php endif; ?>

					</label>
				</div>
	            <div align="center">
					<input <?php if( in_array($icon->id, $selected_icons_array ) ) echo 'checked="checked"'; ?> style="margin:0;" type="checkbox" name="_selected_icons[]" id="icon<?php echo $icon->id; ?>" value="<?php echo $icon->id; ?>" />
				</div>
			</li>
			<?php
			$i++;
...
class Cnss_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
	 		'cnss_widget', // Base ID
			'Social Icon', // Name
			array( 'description' => __( 'Social Icon Widget for sidebar' ) ) // Args
		);
	}
	...
	public function form( $instance ) {
		...
		<table width="100%" border="0">
          <tr>
			  <td><label for="<?php echo $this->get_field_id( 'height' ); ?>"><?php _e( 'Icon Height/Font size<em>(px)</em>:' ); ?></label>
	...
	public function _cn_social_icon_widget( $selected_icons_array = array() ) {
		...
		foreach($video_info as $icon)
		{
			...
			*/
			...
			?><li style="display:inline-block; padding:2px 8px;">
	            <div align="center">
					<label for="<?php echo $this->get_field_id( 'selected_icons'.$icon->id ); ?>">
						<?php if ($image_url === ''): ?>
							<i class="social-plugin fa <?= $fa_array['class']?>" style="color:<?=$fa_array['colour']?>;font-size:<?= $cnss_height ?>px;"></i>
						<?php else:?>
							<img src="<?php echo $image_url?>" border="0" width="<?php echo $cnss_width ?>" height="<?php echo $cnss_height ?>" alt="<?php echo $icon->title ?>" />
	            ...
	            <div align="center">
					<input <?php if( in_array($icon->id, $selected_icons_array ) ) echo 'checked="checked"'; ?> style="margin:0;" type="checkbox" name="<?php echo $this->get_field_name( 'selected_icons' ); ?>[]" id="<?php echo $this->get_field_id( 'selected_icons'.$icon->id ); ?>" value="<?php echo $icon->id; ?>" />
				</div>
			</li>
			<?php
			$i++;
