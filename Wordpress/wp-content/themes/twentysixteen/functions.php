...
// Changes width of id column to 2em.
add_action('admin_head', 'custom_admin_styles');
function custom_admin_styles() {
  echo '<style>
    .column-revealid_id { width: 2em;}
  </style>';
}
...
function sidebar_custom_walker( $args ) {
    return array_merge( $args, array(
        'walker' => new wp_sidebar_menu(),
        // another setting go here ... 
    ) );
}

add_filter( 'wp_nav_menu_args', 'sidebar_custom_walker' );

	class wp_sidebar_menu extends Walker_Nav_Menu {
		/**
...
function custom_theme_content_image_sizes_attr( $sizes, $size ) {
	...
	if ( 'page' === get_post_type() ) {
		...
		840 > $width && 600 <= $width && $sizes = '(max-width: 709px) 86vw, (max-width: 909px) 69vw, (max-width: 984px) 61vw, (max-width: 1362px) 45vw, 600px';
		600 > $width && $sizes = '(max-width: ' . $width . 'px) 85vw, ' . $width . 'px';
	}
